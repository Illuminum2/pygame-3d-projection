...
class Camera:
    ...
    def rotate(self, deltaAngleDegrees, axis): # https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation
        # Rotation relative to camera orientation
        deltaAngle = math.radians(deltaAngleDegrees) / 2
        deltaQ = [
            math.cos(deltaAngle),
            axis[0] * math.sin(deltaAngle),
            axis[1] * math.sin(deltaAngle),
            axis[2] * math.sin(deltaAngle)]
